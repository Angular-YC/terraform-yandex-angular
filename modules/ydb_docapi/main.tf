...
resource "yandex_ydb_database_serverless" "main" {
  ...
  description = "YDB serverless database for ${var.app_name} response cache metadata"
...
# - response_cache_entries: rendered response metadata + TTL
# - response_cache_tags: tag index for purge operations
# - response_cache_locks: lock rows for concurrent render protection
